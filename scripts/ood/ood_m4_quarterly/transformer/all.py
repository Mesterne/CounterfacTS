...
if __name__ == '__main__':
    ...
    generated_train_seas_inc = change_seasonality(train_data, config["sp"], k=0.01)
    generated_train_seas_dec= change_seasonality(train_data, config["sp"], k=2)
    generated_train_slope_dec = change_slope(train_data, config["sp"], m=-10)
    generated_train_lin_dec = change_trend_lin(train_data, config["sp"], h=0.01)
    expanded_train = add_to_dataset(train_data, generated_train_seas_inc)
    expanded_train = add_to_dataset(expanded_train, generated_train_seas_dec)
    expanded_train = add_to_dataset(expanded_train, generated_train_slope_dec)
    expanded_train = add_to_dataset(expanded_train, generated_train_lin_dec)

    generated_val_seas_inc = change_seasonality(validation_data, config["sp"], k=0.01)
    generated_val_seas_dec = change_seasonality(validation_data, config["sp"], k=2)
    generated_val_slope_dec = change_slope(validation_data, config["sp"], m=-1)
    generated_val_lin_dec = change_trend_lin(validation_data, config["sp"], h=0.01)
    expanded_val = add_to_dataset(validation_data, generated_val_seas_inc)
    expanded_val = add_to_dataset(expanded_val, generated_val_seas_dec)
    expanded_val = add_to_dataset(expanded_val, generated_val_slope_dec)
    expanded_val = add_to_dataset(expanded_val, generated_val_lin_dec)
