...
def add_to_dataset(original_data, generated_data):
    for generated, original in zip(generated_data, itertools.cycle(original_data)):
...
def change_seasonality(data, sp, k):
    generated = []
    for ts in tqdm(data):
        ...
        new_seasonal = manipulate_seasonal_determination(decomp.seasonal, k=k)
        new_ts = decomp.trend + new_seasonal + decomp.resid
...
if __name__ == '__main__':
    ...
    generated_train_seas = change_seasonality(train_data, config["sp"], k=0.01)
    generated_train_slope_dec = change_slope(train_data, config["sp"], m=-10)
    generated_train_slope_inc = change_slope(train_data, config["sp"], m=10)
    generated_train_trend_str_inc = change_trend_str(train_data, config["sp"], f=2)
    generated_train_trend_lin_dec = change_trend_lin(train_data, config["spt"], h=0.01)
    expanded_train = add_to_dataset(train_data, generated_train_seas)
    expanded_train = add_to_dataset(expanded_train, generated_train_slope_dec)
    expanded_train = add_to_dataset(expanded_train, generated_train_slope_inc)
    expanded_train = add_to_dataset(expanded_train, generated_train_trend_str_inc)
    expanded_train = add_to_dataset(expanded_train, generated_train_trend_lin_dec)

    generated_val_seas = change_seasonality(validation_data, config["sp"], k=0.01)
    generated_val_slope_dec = change_slope(validation_data, config["sp"], m=-1)
    generated_val_slope_inc = change_slope(validation_data, config["sp"], m=1)
    generated_val_trend_str_inc = change_trend_str(validation_data, config["sp"], f=2)
    generated_val_trend_lin_dec = change_trend_lin(validation_data, config["spt"], h=0.01)
    expanded_val = add_to_dataset(validation_data, generated_val_seas)
    expanded_val = add_to_dataset(expanded_val, generated_val_slope_dec)
    expanded_val = add_to_dataset(expanded_val, generated_val_slope_inc)
    expanded_val = add_to_dataset(expanded_val, generated_val_trend_str_inc)
    expanded_val = add_to_dataset(expanded_val, generated_val_trend_lin_dec)
