...
from src.utils.features import decomps_and_features, extract_trend
...
if __name__ == '__main__':
    # configure script
    model_names = ["nbeats_g", "feedforward", "seq2seq", "tcn", "transformer"]
    dataset_name = "m4_monthly"
    folder_suffix = "real_hold_out"
    datadir = f"data/{dataset_name}"

    with open(os.path.join(f"experiments/{dataset_name}/nbeats_g", "config.yaml"), "r") as f:
        config = yaml.load(f, Loader=yaml.FullLoader)

    print("Setting up new dataset")
    # set up new dataset
    ...
    _, full_features = decomps_and_features(train_data, sp=config["sp"])
    norm_full_length_features = scaler.transform(full_features)
    ...
    for model_name in model_names:
        ...
        train_dataloader = create_train_dataloader(train_data, config["trainer_args"]["context_length"], config["trainer_args"]["prediction_length"],
                                                config["trainer_args"]["batch_size"], config["trainer_args"]["num_batches_per_epoch"])

        validation_dataloader = create_validation_dataloader(validation_data, config["trainer_args"]["context_length"], config["trainer_args"]["prediction_length"],
                                                            config["trainer_args"]["batch_size"], config["trainer_args"]["num_batches_per_epoch"])
